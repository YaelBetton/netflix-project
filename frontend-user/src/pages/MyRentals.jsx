...
function MyRentals() {
  ...
  const now = new Date();
  const getExpiryDate = (rental) => {
    if (rental.expiryDate) {
      return new Date(rental.expiryDate);
    }

    const rentalDate = new Date(rental.rentalDate || Date.now());
    rentalDate.setDate(rentalDate.getDate() + 7);
    return rentalDate;
  };

  // Filtrer les locations actives (non expirees)
  const activeRentals = rentals.filter((rental) => getExpiryDate(rental) > now);
  // Filtrer les locations expirees
  const expiredRentals = rentals.filter((rental) => getExpiryDate(rental) <= now);
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {activeRentals.map((rental) => (
                <div key={rental.id} className="group relative">
                  <Link to={`/movie/${rental.movieId || rental.id}`}>
                    <img
                      src={rental.poster}
                      alt={rental.title}
                    ...
                    <div className="mt-2">
                      ...
                      <p className="text-sm text-green-400">
                        Expire dans {Math.max(0, Math.ceil((getExpiryDate(rental) - now) / (1000 * 60 * 60 * 24)))} jour(s)
                      </p>
                    </div>
                  </Link>
                </div>
